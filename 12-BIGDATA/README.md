...
```
По результатм тестирования можно сделать вывод, что скорость загрузки данных в БД PostgreSQL из файла csv оказалась существенно меньше, чем в БД MySQL, а скорость выполнения запросов, 
в которых используется сканирование всей таблицы, соизмерима. 
В БД PostgreSQL потребовалось больше места на файловой системе для хранения одного и того же объёма данных, что и в БД MySQL.
